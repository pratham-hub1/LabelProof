...
def score_record(result: dict, truth: dict) -> dict:
    ...
    if 'R8' in expected:
        ...
        try:
            ...
            r7_table = [{"max_area": 50, "normal": 1.0, "molded": 2.0}, {"max_area": 100, "normal": 1.5, "molded": 3.0}, {"max_area": 2500, "normal": 2.5, "molded": 4.0}, {"max_area": 999999, "normal": 4.0, "molded": 6.0}]
            
        pdp_width = truth.get('pdp_width_mm', 0)
        pdp_height = truth.get('pdp_height_mm', 0)
        pdp_area = (pdp_width * pdp_height) / 100 # cm2
        r_min, r_max = get_required_mm_candidates(pdp_area, False, r7_table)
        req_height = r_min
